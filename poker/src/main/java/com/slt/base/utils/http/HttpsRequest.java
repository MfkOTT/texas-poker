...
public class HttpsRequest extends HttpConfig implements IRequest {
	...
	public String doPost(String url, String content, String ctype, String charset,Map<String,String> headMap) throws Exception {
		...
		return doPost(url, ctype + ";charset=" + useCharset, reqContent,headMap);
	}
	...
	public String doPostFile(String url, String fileName, byte[] content, String charset,Map<String,String> headMap) throws Exception {
		...
		try {
			conn = (HttpsURLConnection) getConnection(getHttpsUrl(url), METHOD_POST, "multipart/form-data; boundary=" + boundary,headMap);
	...
	private String doPost(String url, String ctype, byte[] content,Map<String,String> headMap) throws Exception {
		HttpsURLConnection conn = null;
		OutputStream out = null;
		String rsp = null;
		try {
			conn = (HttpsURLConnection) getConnection(getHttpsUrl(url), METHOD_POST, ctype,headMap);
			conn.setSSLSocketFactory(sslConfig.initSSLSocketFactory());
			conn.setHostnameVerifier(new HttpSSLConfig.DefaultHostnameVerifier());
	...
	public String doGet(String url) throws Exception {
		...
		String rsp = null;
		try {
			conn = (HttpsURLConnection) getConnection(getHttpsUrl(url), METHOD_GET, null,null);
			conn.setSSLSocketFactory(sslConfig.initSSLSocketFactory());
			conn.setHostnameVerifier(new HttpSSLConfig.DefaultHostnameVerifier());
